...
inline float tanhFast(const float x) noexcept {
...
inline float softClipFast(const float x, const float limit = 0.95f) noexcept {
...
inline float exp2Fast(const float x) noexcept {
...
inline float tanPrewarpFast(const float w) noexcept {
    const float w2 = w * w;
